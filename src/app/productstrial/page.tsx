...
const page = ({}) => {
  ...
  const CreateProductPost = api.product.create.useMutation({
    onSuccess: () => {
      router.refresh();
      setName("");
    },
  });
  const isCreatePage = pathname === "/profile/post";
    ...
    const imageUrl = form.watch("images")
    ...
    return(
        <>
          ...
          onSubmit = {form.handleSubmit(async (values)=>{
            const newValues = {...values, price: parseFloat(values.price)}
            CreateProductPost.mutate(newValues);
          })}>
              {!!imageUrl ? (
